...
export const CardProduct: FC<Props> = ({ product }) => {
  return (
    <>
      <div className=" rounded-lg shadow-md bg-gray-800 h-full w-full">
...
export const DetailProduct = ({ product }: { product: IProduct }) => {
  ...
  const handlerIncrement = () => setCurrent(current <= product.images.length - 2 ? current + 1 : 0);
  const handlerDecrement = () => setCurrent(current > 0 ? current - 1 : product.images.length - 1);

  return (
    <Modal
      width="max-w-4xl"
      title={product.title}
      handleClick={async () => true}
      ButtonOpen={({ onClick }) => (
        <button
          onClick={onClick}
      ...
      <div id="default-carousel" className="relative" data-carousel="static">
        ...
        <div className="overflow-hidden relative h-56 rounded-lg sm:h-64 xl:h-80 2xl:h-96">
          {product.images.map((im) => (
            <div
              className={` ${product.images[current] !== im ? 'hidden' : ''} duration-700 ease-in-out`}
              data-carousel-item
              key={im}
            >
        ...
        <div className="flex absolute bottom-5 left-1/2 z-30 space-x-3 -translate-x-1/2">
          {product.images.map((im, idx) => (
            <button
              ...
              key={im}
              type="button"
              className={`w-3 h-3 rounded-full ${im === product.images[current] ? 'bg-indigo-500' : 'bg-gray-900'}`}
              aria-current="false"
              aria-label="Slide 1"
